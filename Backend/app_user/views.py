...
class Avatar(APIView):
    def post(self, request):
        avatar = Profile.objects.get_or_create(
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            if not user.check_password(serializer.data.get("passwordCurrent")):
                return Response(
                    data={"passwordCurrent": "Не верный пароль"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            elif serializer.data.get("password") != serializer.data.get(
                "passwordReply"
            ):
                return Response(
                    data={"password": "Не верный пароль"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            user.set_password(serializer.data.get("password"))
            user.save()
            return Response(
                data={"password": serializer.data.get("password")},
